...
#ifndef __AI_SINE_APPROX_H__
#define __AI_SINE_APPROX_H__
#pragma once

#include "ai_platform.h"
#include "ai_platform_interface.h"

#define AI_SINE_APPROX_MODEL_NAME          "sine_approx"

#define AI_SINE_APPROX_ACTIVATIONS_ALIGNMENT   (4)

#define AI_SINE_APPROX_IN_NUM       (1)
#define AI_SINE_APPROX_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_SINE_APPROX_IN_SIZE { \
  (1 * 1 * 1), \
}
#define AI_SINE_APPROX_IN_1_SIZE  (1 * 1 * 1)
#define AI_SINE_APPROX_IN_1_SIZE_BYTES  ((1 * 1 * 1) * 4)





#define AI_SINE_APPROX_OUT_NUM      (1)
#define AI_SINE_APPROX_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 1, 1, NULL), \
}
#define AI_SINE_APPROX_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_SINE_APPROX_OUT_1_SIZE  (1 * 1 * 1)
#define AI_SINE_APPROX_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 4)

#define AI_SINE_APPROX_N_NODES (5)

AI_API_DECLARE_BEGIN

/*!
 * @defgroup sine_approx
 ...
 * @ref ai_sine_approx_create. Both the activations memory buffer 
 ...
 * using @ref ai_sine_approx_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_SINE_APPROX_*
 * defines into file @ref sine_approx.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_sine_approx_forward(
  ai_handle network, const ai_buffer* input);

AI_API_DECLARE_END

#endif /*__AI_SINE_APPROX_H__*/
