...
#define AI_NET_OBJ_INSTANCE g_sine_approx
 
#undef AI_SINE_APPROX_MODEL_SIGNATURE
#define AI_SINE_APPROX_MODEL_SIGNATURE     "eddc2c5b88d59df5a62fa61f4ebade8b"
...
#define AI_TOOLS_DATE_TIME   "Tue Jan 17 18:28:55 2023"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_SINE_APPROX_N_BATCHES
#define AI_SINE_APPROX_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 128, 1,
                     NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SINE_APPROX_IN_NUM, &serving_default_dense_input0_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_SINE_APPROX_OUT_NUM, &dense_2_output),
  &dense_0_layer, 0, NULL)



AI_DECLARE_STATIC
ai_bool sine_approx_configure_activations(
  ai_network* net_ctx, const ai_buffer* activation_buffer)
{
  AI_ASSERT(net_ctx &&  activation_buffer && activation_buffer->data)

  ai_ptr activations = AI_PTR(AI_PTR_ALIGN(activation_buffer->data, AI_SINE_APPROX_ACTIVATIONS_ALIGNMENT));
...
AI_DECLARE_STATIC
ai_bool sine_approx_configure_weights(
...
AI_API_ENTRY
ai_bool ai_sine_approx_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_SINE_APPROX_MODEL_NAME,
      .model_signature   = AI_SINE_APPROX_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_sine_approx_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_sine_approx_create(
...
AI_API_ENTRY
ai_handle ai_sine_approx_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_sine_approx_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= sine_approx_configure_weights(net_ctx, &params->params);
  ok &= sine_approx_configure_activations(net_ctx, &params->activations);
...
AI_API_ENTRY
ai_i32 ai_sine_approx_forward(ai_handle network, const ai_buffer* input)
{
